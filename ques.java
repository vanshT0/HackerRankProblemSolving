...
public static int diagonalDifference(List<List<Integer>> arr) {
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m;j++){
            if(i == j) d1 = d1 + arr.get(i).get(j);
            if(i + j == n-1) d2 = d2 + arr.get(i).get(j);
        }
    }
...
public static void plusMinus(List<Integer> ar) {
    ...
    double counto = 0;
    for(int i = 0; i < ar.size();i++){
        if(ar.get(i) < 0) countN++;
        if(ar.get(i) > 0) countP++;
        if(ar.get(i) == 0)counto++;
    }
...
public static void staircase(int n) {
    ...
    for(int i = 0; i < arr.size();i++){
        if(arr.get(i) > maxEle) maxEle = arr.get(i);
      
    }
    for(int i = 0; i < arr.size();i++){
        if (arr.get(i) == maxEle) count++;
    }
     return count; 
}
...
public static void countApplesAndOranges(int sD, int tD, int aL, int bL, List<Integer> apples, List<Integer> oranges) {
...
public static int migratoryBirds(List<Integer> ar) {
    int c1 = 0; int c2= 0;  int c3 = 0;int c4 = 0;int c5 = 0; int fans = 0;
       for(int i = 0; i < ar.size(); i++){
           if(ar.get(i) == 1) c1++;
           if(ar.get(i) == 2) c2++;
           if(ar.get(i) == 3) c3++;
  ...
  public static void bonAppetit(List<Integer> ar, int k, int b) {
    ...
    if((sum - ar.get(k)) / 2 == b) System.out.print("Bon Appetit");
    else System.out.print(Math.abs(b-(sum - ar.get(k)) / 2) );
...
public static int pageCount(int n, int p) {
    ...
    public static int countingValleys(int steps, String path) {
        steps = path.length();
                    ...
                    int sumChecker = 0; int ans  = 0;
                    for(int i = 0; i < steps;i++){
                        char ch = path.charAt(i);
                        if(ch == 'D') sumChecker--;
                       if(ch == 'U')sumChecker++;
    
                        if(sumChecker == 0 && path.charAt(i) == 'U') ans++;
                    }
                    return ans;
...
public static int formingMagicSquare(List<List<Integer>> arr) {
    List<Integer> row = new ArrayList<Integer>();
        ...
        int allMagic[][] = { { 4, 9, 2, 3, 5, 7, 8, 1, 6 }, { 4, 3, 8, 9, 5, 1, 2, 7, 6 }, { 2, 9, 4, 7, 5, 3, 6, 1, 8 },
    ...
    public static int pickingNumbers(List<Integer> arr) {
       ...
       public static int hurdleRace(int k, List<Integer> height) {
        ...
        for(int i = 0; i < height.size(); i++){
            ...
            if(maxEle < height.get(i)) maxEle = height.get(i); 
            
            }
            if(k >= maxEle) return 0;
...
public static int saveThePrisoner(int n, int m, int s) {
 ...
 public static void main(String[] args) {
    System.out.print(saveThePrisoner(5,2,1));
